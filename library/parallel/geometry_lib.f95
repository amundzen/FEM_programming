...
! modified by lm 23/02/00 by the addition of 20bxzv

!------------------  g_t to g type routines ---------------------------------
 ...
 IMPLICIT NONE
 INTEGER,INTENT(IN)::nod,g_t(:); INTEGER,INTENT(OUT)::g(:); INTEGER:: i
             DO i=1,nod;g(i)=g_t(4*i-3); g(i+28)=g_t(4*i-1)
                        g(i+48)=g_t(4*i) ; END DO
             DO i=1,4 ; g(i+20) = g_t(8*i-6)
                        g(i+24) = g_t(8*i+42); END DO
 END SUBROUTINE g_t_g_ns

!------- some simple boundary condition routines for cuboids ----------------
 SUBROUTINE box_bc8(nxe,nye,nze,rest)
 ...
 face = (nxe+1)*(nze+1) ; count = 0
 DO i = 0 , nye - 1
    DO j = i*face+1,(i+1)*face
       IF (j/(nxe+1)*(nxe+1) ==j .OR. j < i*face + nxe + 1) THEN
         count = count + 1 ;  rest(count,1) = j    ; rest(count,2) = 0
       END IF
    END DO
 END DO
 ...
 SUBROUTINE box_bc20(nxe,nye,nze,rest)
 ! boundary conditions for simple 20-node element box
 IMPLICIT NONE
 INTEGER,INTENT(IN)::nxe,nye,nze; INTEGER,INTENT(OUT)::rest(:,:)
 INTEGER:: i,j,k,l,m,n,face,face1,face2,count
 face1 = 3*nxe*nze+2*(nxe+nze)+1 ; face2 = (nxe+1)*(nze+1)
 face = face1 + face2  ;   l = nze*(nxe+1) ; m = 3*nxe + 2
 n = 3*nxe*nze + 2*nze  ; count = 0
 DO i = 0 , nye 
    DO j = i*face+1,i*face+face1
       k = j - i*face
         IF((k+m-1)/m*m==(k+m-1) .AND. k<=n ) THEN
           count = count + 1 ; rest(count,1)=j; rest(count,2)=0; CYCLE 
         ...
         ELSE IF(k/m*m==k .AND. k<=n) THEN
           count = count + 1 ; rest(count,1)=j; rest(count,2)=0; CYCLE 
         ELSE IF (i==0 .OR. i==nye .AND. k<=n) THEN
           count = count + 1 ; rest(count,1)=j; rest(count,2)=0; CYCLE 
         ELSE IF (k>n) THEN
           count = count + 1 ; rest(count,1)=j; rest(count,2)=0  
         END IF
    ...
    END IF
 END DO
 END SUBROUTINE box_bc20
 SUBROUTINE cube_bc20(nxe,nye,nze,rest)
 ...
 SUBROUTINE ns_cube_bc20(nxe,nye,nze,rest)
 ...
 SUBROUTINE nsy_cube_bc20(nxe,nye,nze,rest)
 ! boundary conditions for symmetric Navier-Stokes 20-node element cuboid  
 IMPLICIT NONE
 INTEGER,INTENT(IN)::nxe,nye,nze; INTEGER,INTENT(INOUT)::rest(:,:)
 INTEGER:: i,j,k,l,m, count,node
 node = 0;  count = 0; k = 2*nxe+1; l = nxe+1
 DO m = 0 , nye
        IF(m==0 ) THEN    ! front 
         DO j=1,nze
            DO i=1,k
               ...
               ELSE IF (j==1) THEN
                  node = node +1; count = count + 1
                  rest(count,1)=node; rest(count,2:)=(/1,1,0,0/)
               ELSE IF (j>1.AND.(i==1.OR.i==k)) THEN
                  node = node +1 ; count = count + 1
                  rest(count,1)=node; rest(count,2:)=(/0,1,0,0/)
               ELSE IF (mod(i,2)==0) THEN
                  node = node +1; count = count + 1
                  rest(count,1)=node; rest(count,2:)=(/1,0,0,1/)
               ELSE IF (mod(i,2)/=0) THEN
                  node = node +1; count = count + 1
                  rest(count,1)=node; rest(count,2:)=(/1,1,0,1/)
               END IF
            END DO
            DO i=1,l
               IF(i==1.OR.i==l) THEN
        ...
        END IF
 END DO
 END SUBROUTINE nsy_cube_bc20

!-------------------  Some simple loaded patches ----------------------------

 SUBROUTINE loading(nxe,nze,nle,no,val)
! simple loading of a square patch  nle  of surface width
 IMPLICIT NONE
 INTEGER,INTENT(IN)::nxe,nze,nle; INTEGER,INTENT(OUT)::no(:)
 REAL(iwp),INTENT(OUT)::val(:)  ;  INTEGER::f1,f2,f3,count,i,j
 f1 = 6*nxe*nze; f2 = (3*nxe+1)*nze; f3 = (9*nxe+2)*nze; count=1
...
! simple loading of a square  Biot patch  nle  of surface width
 ...
 SUBROUTINE ns_loading(nxe,nye,nze,no)
! simple lid displacement of a square  Navier-Stokes patch  
 IMPLICIT NONE
 INTEGER,INTENT(IN)::nxe,nye,nze; INTEGER,INTENT(OUT)::no(:)
 INTEGER::f1,f2,f3,count,i,j
 f1 = 3*nxe+1 + (nxe+1)*nze; f2 =3*(nxe-1)*(nze-1)+(nxe+1)
 f3 = 10*nxe*nze -3*nxe - 5*nze + 4; no(1) = 1; count=2
 DO  i = 1,nxe   ! front face
   no(count) = 3*i -1; count=count+1; no(count)=3*i;count = count + 1
 END DO
 DO j = 0 , nye-1    ! other face pairs
   DO i = 1 , nxe + 1   ! mid - planes
    no(count) = i + f1 + j*(f2 + f3)    ;    count = count + 1
   END DO
   no(count) = f1+(j+1)*f2+j*f3 +1; count = count + 1
   DO i = 1 , nxe 
    no(count) = 3*i-1 + f1 +(j+1) * f2 + j * f3   ;   count = count + 1
    no(count) = 3*i + f1 +(j+1) * f2 + j * f3   ;   count = count + 1
   END DO
  END DO
 END SUBROUTINE ns_loading

 SUBROUTINE nsy_loading(nxe,nye,nze,no)
! simple lid displacement of a symmetric  Navier-Stokes patch  
 IMPLICIT NONE
 INTEGER,INTENT(IN)::nxe,nye,nze; INTEGER,INTENT(OUT)::no(:)
 INTEGER::f1,f2,f3,count,i,j
 f1 = 7*nxe*nze-nxe-3*nze+3 ; f2 =3*(nxe-1)*(nze-1)+(nxe+1)
 f3 = 10*nxe*nze -3*nxe - 5*nze + 4; no(1) = 1; count=2
 DO  i = 1,nxe   ! front face
   no(count) = 3*i -1; count=count+1; no(count)=3*i;count = count + 1
 END DO
 DO j = 0 , nye    ! other face pairs
   DO i = 1 , nxe + 1   ! mid - planes
    no(count) = i + f1 + j*(f2 + f3)    ;    count = count + 1
   END DO
   no(count) = f1+(j+1)*f2+j*f3 +1; count = count + 1
...
! a line of (nonuniform) beam elements
 implicit none
  ...
  real(iwp),intent(in)::ell; real(iwp),intent(out)::coord(:,:)
  num=(/iel,iel+1/)
...
!      counting in the x-direction    ; local numbering clockwise
  implicit none
  real(iwp),intent(in):: aa,bb; integer,intent(in):: iel,nxe
  real(iwp),intent(out) :: coord(:,:); integer,intent(out):: num(:)
  integer::ip,iq,jel
...
!      counting in the y-direction ; local numbering clockwise
 implicit none
 real(iwp),intent(in) :: width(:),depth(:) ; real(iwp),intent(out) :: coord(:,:)
...
implicit none
integer,intent(in)::iel,nxe; real(iwp),intent(in)::aa,bb
real(iwp),intent(out)::coord(:,:); integer,intent(out)::num(:)
integer :: ip,iq    ; iq=(iel-1)/nxe+1; ip=iel-(iq-1)*nxe
 ...
 return
   ...
   implicit none
   integer,intent(in)::iel,nye; real(iwp),intent(in)::aa,bb
   real(iwp),intent(out)::coord(:,:);integer,intent(out)::num(:)
...
!  mesh of 4-node quad elements numbering in the y-direction
  implicit none
  real(iwp),intent(in)::width(:),depth(:); integer,intent(in)::iel,nye
  real(iwp),intent(out)::coord(:,:); integer,intent(out):: num(:)
...
!  mesh of 8-node quad elements numbering in the y-direction
  implicit none
  real(iwp),intent(in):: aa,bb ; integer,intent(in):: iel,nye
  real(iwp),intent(out)::coord(:,:); integer,intent(out):: num(:)
  integer:: ip,iq; ip=(iel-1)/nye+1; iq=iel-(ip-1)*nye
...
implicit none
  integer,intent(in)::iel,nxe;real(iwp),intent(in)::width(:),depth(:)
  real(iwp),intent(out)::coord(:,:); integer,intent(out)::num(:)
  integer::ip,iq; iq=(iel-1)/nxe+1; ip=iel-(iq-1)*nxe
  num(1)=iq*(3*nxe+2)+2*ip-1; num(2)=iq*(3*nxe+2)+ip-nxe-1
...
!      for boxes of 8-node brick elements counting x-z planes in y-direction
 implicit none
   ...
   real(iwp),intent(in)::aa,bb,cc; real(iwp),intent(out)::coord(:,:)
...
implicit none
  integer,intent(in)::iel,nxe,nze; real(iwp),intent(in)::aa,bb,cc
  real(iwp),intent(out)::coord(:,:); integer,intent(out)::num(:)
...
implicit none
  ...
  num(15)=num(13)-3*nxe-2; num(16)=num(15)+1; num(17)=num(16)+1
  num(18)=num(14)+1; num(19)=num(13)+2; num(20)=num(13)+1 
  coord(1:3,1)=width(ip); coord(9:10,1)=width(ip); coord(13:15,1)=width(ip)
  coord(5:7,1)=width(ip+1); coord(11:12,1)=width(ip+1); coord(17:19,1)=width(ip+1)
  coord(4,1)=.5*(coord(3,1)+coord(5,1));coord(8,1)=.5*(coord(1,1)+coord(7,1))
  coord(16,1)=.5*(coord(15,1)+coord(17,1))
  coord(20,1)=.5*(coord(13,1)+coord(19,1))
  coord(1:8,2)=depth(iq); coord(13:20,2)=depth(iq+1)
  coord(9,2)=.5*(coord(1,2)+coord(13,2))
  coord(10,2)=.5*(coord(3,2)+coord(15,2))
  coord(11,2)=.5*(coord(5,2)+coord(17,2))
  coord(12,2)=.5*(coord(7,2)+coord(19,2))
  coord(1,3)=-height(is+1); coord(7:9,3)=-height(is+1); coord(12:13,3)=-height(is+1)
  coord(19:20,3)=-height(is+1); coord(3:5,3)=-height(is)
  coord(10:11,3)=-height(is); coord(15:17,3)=-height(is)
  coord(2,3)=.5*(coord(1,3)+coord(3,3))
  coord(6,3)=.5*(coord(5,3)+coord(7,3))
  coord(14,3)=.5*(coord(13,3)+coord(15,3))
  coord(18,3)=.5*(coord(17,3)+coord(19,3))
...
!    (numbering in the y-direction)
 implicit none
  real(iwp),intent(in):: top(:),bot(:),depth(:); real(iwp),intent(out)::coord(:,:)
  integer,intent(in):: iel,nye; integer,intent(out)::num(:)
  real(iwp) :: fac1 , fac2  ; integer :: ip,iq  
...
implicit none
real(iwp),intent(in)::width(:),angs(:),surf(:),down ;real(iwp),intent(out)::coord(:,:)
integer,intent(in)::iel,nxe,fixed_seep,fixed_down;integer,intent(out)::num(:)
real(iwp)::angr(size(angs)),pi,b1,b2,tan1,tan2,fac1,fac2
integer::ip,iq
 ...
 b1=(surf(ip)-down)/real(fixed_seep+1,iwp); b2=(surf(ip+1)-down)/real(fixed_seep+1,iwp)
 ...
 b1=real(fixed_down+fixed_seep-iq,iwp)/real(fixed_down-1,iwp) 
 b2=real(fixed_down+fixed_seep-iq+1,iwp)/real(fixed_down-1,iwp) 
...
!---------------------Construction and excavation -----------------------------
 ...
 real(iwp),intent(out)::g_coord(:,:);integer::ig,i,j,ii,lnye
 ...
 real(iwp),intent(out)::g_coord(:,:);integer::ig,i,j
 real(iwp),intent(in)::width(:),depth(:)
 ig = 0
 do i=1,nye; do j=1,nxe
 ig = ig + 1
